...
const ProfileView = ({ user, videos, onUpdateUser }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-4">
          ...
          <div className="flex justify-center md:justify-start gap-8 mb-6">
            ...
            <div className="text-center md:text-left group cursor-help" title="Collect points by voting, uploading, and commenting!">
              <div className="flex items-center gap-1.5 justify-center md:justify-start mb-[-2px]">
                ...
                <span className="block text-2xl font-black text-yellow-500 tabular-nums">{user.points?.toLocaleString() || 0}</span>
              </div>
      ...
      <h3 className="bungee text-xl mb-6 flex items-center gap-3 uppercase">
        MY TRIPS
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {videos.map(v => (
          <div key={v.id} className="group relative aspect-[9/16] bg-zinc-900 rounded-2xl overflow-hidden cursor-pointer">
            <img src={v.thumbnailUrl} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" alt="" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all flex flex-col justify-end p-4">
              <span className="text-[10px] font-black text-purple-400 uppercase tracking-tighter">{v.trips} Trips</span>
              <p className="text-xs text-white font-bold truncate">{v.title}</p>
            </div>
          </div>
        ))}
