...
const Navbar = ({ user, onAuthClick, onUploadClick, onViewChange, currentView, onLogout }) => {
  return (
    <nav className="sticky top-0 z-50 bg-black/80 backdrop-blur-md border-b border-zinc-800 px-4 md:px-8 py-3 flex justify-between items-center gap-4">
      <div 
      ...
      <div className="hidden lg:flex flex-1 max-w-md mx-8">
        <div className="relative w-full group">
          <input 
            ...
            className="w-full bg-zinc-900 border border-zinc-800 rounded-full py-2 px-10 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500/50 text-sm transition-all text-white"
          />
      ...
      <div className="flex items-center gap-4">
        ...
        {user ? (
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 bg-zinc-900 border border-zinc-800 px-3 py-1.5 rounded-full shadow-inner group transition-colors cursor-help" title="Your Trippin' Points">
              ...
              <span className="text-white font-black text-xs tabular-nums">{user.points?.toLocaleString() || 0}</span>
            </div>
            ...
            <div className="group relative">
              ...
              <div className="absolute right-0 top-12 w-56 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all translate-y-2 group-hover:translate-y-0">
                <div className="px-4 py-2 border-b border-zinc-800 mb-2">
                  <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest text-[9px]">Signed in as</p>
...
const LogoutIcon = ({className}) => <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>;
